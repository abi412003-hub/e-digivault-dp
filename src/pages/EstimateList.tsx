...
function EstimateCard({
  ...
  const status = est.status ?? 'Not Started';
  ...
  return (
    <div
      onClick={handleTap}
      className="bg-background rounded-2xl border border-muted shadow-sm p-4 flex items-center gap-3 cursor-pointer active:bg-muted/30 transition-colors"
    >
      {/* Photo */}
      ...
      <div className="flex-1 min-w-0 space-y-0.5">
        <p className="text-[16px] font-bold text-foreground truncate">
          {est.property_name || est.property || '—'}
        </p>
        <p className="text-[12px] text-muted-foreground">
          Property ID : {est.property || est.name}
        </p>
        <p className="text-[12px]">
          <span className="text-muted-foreground">Service: </span>
          <span className="text-[#3B82F6] font-medium">{est.service_name || est.service || '—'}</span>
        </p>
        <p className="text-[12px] text-muted-foreground">Date: {est.date ?? '—'}</p>
        <p className="text-[12px]">
...
export default function EstimateList() {
  ...
  useEffect(() => {
    ...
    fetchList(
      'DigiVault Estimate',
      ['name', 'property', 'property_name', 'service', 'service_name', 'date', 'status', 'property_photo'],
      [['client', '=', clientId]],
      100,
      'creation desc'
    )
  ...
  const allowedStatuses = STATUS_FILTERS[activeTab];
  const tabFiltered = estimates.filter((e) =>
    allowedStatuses.some((s) => (e.status ?? '').toLowerCase() === s.toLowerCase())
  );
  ...
  return (
    <div className="min-h-svh bg-background pb-20">
      ...
      <div className="px-4">
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-full text-[14px] font-medium transition-colors ${
                activeTab === tab
                  ...
                  : 'bg-background border border-[#1A3C8E] text-[#1A3C8E]'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="mt-4 space-y-3">
          {loading ? (
            ...
            filtered.map((est) => (
              <EstimateCard key={est.name} est={est} clientId={clientId} navigate={navigate} />
            ))
          )}
        </div>
      </div>
      ...
      {/* FAB */}
      <button
        onClick={() => navigate(`/estimate-builder?client=${clientId}&property=new`)}
        className="fixed bottom-20 right-4 w-14 h-14 rounded-full bg-[#3B82F6] text-white shadow-lg flex items-center justify-center z-30 active:scale-95 transition-transform"
      >
