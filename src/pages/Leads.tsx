...
import { ArrowLeft, MessageSquare, Bell, User, BarChart3, CheckCircle, XCircle, Clock, Plus, Eye, History } from 'lucide-react';
import BottomNav from '@/components/BottomNav';

interface Lead {
  name: string;
  lead_name: string;
  lead_status: string; // correct field name
  creation: string;
}
...
export default function Leads() {
  ...
  useEffect(() => {
    if (!dp_id) return;
    // created_by is the correct link field (not assigned_to)
    fetchList('DigiVault Lead', ['name','lead_name','lead_status','creation'], [['created_by','=',dp_id]], 100, 'creation desc')
      .then((d: any[]) => {
        if (!d) return;
        setLeads(d);
        setCounts({
          verified: d.filter((l) => l.lead_status === 'Verified').length,
          rejected: d.filter((l) => l.lead_status === 'Rejected').length,
          pending:  d.filter((l) => !['Verified','Rejected'].includes(l.lead_status)).length,
        });
      })
      .catch(() => {});
  }, [dp_id]);
  ...
  const activityRows = leads.length > 0
    ? leads.slice(0,20).map((l) => ({ title: l.lead_name || l.name, date: fmtDate(l.creation), status: l.lead_status }))
    : [
      ...
      <div className="px-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-green-500 flex items-center justify-center shrink-0"><CheckCircle className="w-5 h-5 text-white" /></div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-red-500 flex items-center justify-center shrink-0"><XCircle className="w-5 h-5 text-white" /></div>
        ...
        <div className="flex justify-center mt-3">
          <div className="w-[60%] bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center shrink-0"><Clock className="w-5 h-5 text-white" /></div>
        ...
        <div className="flex gap-3 mt-5">
          <button onClick={() => navigate('/add-lead')} className="flex-1 bg-[#1A3C8E] text-white rounded-xl py-3 text-[14px] font-bold flex items-center justify-center gap-2">Add new Leads <Plus className="w-4 h-4" /></button>
          <button onClick={() => navigate('/leads-report')} className="flex-1 bg-background border border-[#1A3C8E] text-[#1A3C8E] rounded-xl py-3 text-[14px] font-bold flex items-center justify-center gap-2">Leads Report <Eye className="w-4 h-4" /></button>
        </div>
        ...
        <div className="border border-border rounded-xl overflow-hidden">
          ...
          {activityRows.map((row, idx) => {
            ...
            return (
              <div key={idx} className="grid grid-cols-[1fr_90px_100px] px-3 py-2.5 border-t border-border items-center">
                ...
                <div className="flex justify-end"><span className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${pill.cls}`}>{pill.label}</span></div>
              </div>
            );
          })}
        </div>
