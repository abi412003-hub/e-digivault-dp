...
import { ArrowLeft, MessageSquare, Bell, User, Search, Building2, MapPin, Camera, Info, Wrench } from 'lucide-react';
import BottomNav from '@/components/BottomNav';

function PropertyCard({ property, clientId, navigate }: { property: any; clientId: string; navigate: ReturnType<typeof useNavigate> }) {
  const actions = [
    {
      icon: MapPin, label: 'Locate',
      onClick: () => {
        const lat = property.property_latitude;
        const lng = property.property_longitude;
        if (lat && lng && (lat !== 0 || lng !== 0)) {
          window.open(`https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`, '_blank');
        }
      },
    },
    { icon: Camera,  label: 'Photos',  onClick: () => navigate(`/property-photos?id=${property.name}`) },
    { icon: Info,    label: 'Details', onClick: () => navigate(`/property-details?id=${property.name}`) },
    { icon: Wrench,  label: 'Task',    onClick: () => navigate(`/task-details?property=${property.name}&client=${clientId}`) },
  ];
  ...
  return (
    <div className="bg-background rounded-2xl border border-muted shadow-sm overflow-hidden">
      <div className="bg-[#f0f4ff] px-4 py-5 flex flex-col items-center gap-1">
        ...
        <h3 className="text-[18px] font-bold text-foreground mt-1">{property.property_name || property.name}</h3>
        {/* property_type is the correct field (project doesn't exist) */}
        <p className="text-[12px] text-muted-foreground">{property.property_type || 'Property'} — {property.name}</p>
      </div>
      <div className="grid grid-cols-4 border-t border-muted">
        {actions.map((action, i) => (
          <button key={action.label} onClick={action.onClick} className={`flex flex-col items-center justify-center gap-1 py-3 ${i < 3 ? 'border-r border-muted' : ''} active:bg-muted/30 transition-colors`}>
...
export default function Properties() {
  ...
  useEffect(() => {
    ...
    // Correct fields: property_name, property_type, property_latitude, property_longitude
    // 'project' field does NOT exist on DigiVault Property
    fetchList('DigiVault Property',
      ['name','property_name','property_type','client','property_address','property_latitude','property_longitude'],
      [['client','=',clientId]], 50, 'creation desc')
  ...
  const filtered = search.trim()
    ? properties.filter((p) => (p.property_name ?? '').toLowerCase().includes(search.toLowerCase()) || (p.name ?? '').toLowerCase().includes(search.toLowerCase()))
    : properties;
    ...
    <div className="min-h-svh bg-background pb-20">
      ...
      <div className="px-4">
        ...
        <div className="flex items-center gap-2 mt-4 mb-3">
          ...
          <span className="text-[18px] font-bold text-foreground">{filtered.length} Properties</span>
        </div>
        <div className="space-y-3">
          {loading ? (
            ...
            filtered.map((p) => <PropertyCard key={p.name} property={p} clientId={clientId} navigate={navigate} />)
          )}
        </div>
      </div>
      <BottomNav />
