...
export default function TaskDetails() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchOne('DigiVault Task', taskId).catch(() => null),
      fetchList(
        'DigiVault Task Step',
        ['name', 'step_number', 'step_label', 'description', 'status'],
        [['parent_task', '=', taskId]],
        20,
        'step_number asc'
      ).catch(() => []),
      ...
      setSteps(
        (s && s.length > 0)
          ...
          : [
              ...
              { name: 's4', step_number: 4, step_label: 'Step 4', description: 'Case Worker', status: t?.status?.toLowerCase().includes('completed') ? 'Completed' : '' },
            ]
      );

      if (t?.client) {
        fetchOne('DigiVault Client', t.client).then((c) => c && setClient(c)).catch(() => {});
      }
      setLoading(false);
    });
  }, [taskId]);
  ...
  const docTitle = task?.document_title ?? task?.task_title ?? 'Khatha Extract';
  const docImage = task?.completed_document ? getFileUrl(task.completed_document) : '';
  ...
  const handleUpdateStep = async (step: any) => {
    try {
      await updateRecord('DigiVault Task Step', step.name, { status: 'Completed' });
      setSteps((prev) =>
        prev.map((s) => (s.name === step.name ? { ...s, status: 'Completed' } : s))
      );
  ...
  const handleSendForApproval = async () => {
    if (!allStepsDone) {
      toast({ title: 'Complete all steps first' });
      return;
    }
    setSending(true);
    try {
      await updateRecord('DigiVault Task', taskId, { status: 'Pending For Approval' });
      toast({ title: 'Sent for approval' });
      setTask((t: any) => ({ ...t, status: 'Pending For Approval' }));
    } catch {
    ...
    <div className="min-h-svh bg-background pb-20">
      ...
      <div className="px-4">
        ...
        <div className="py-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] text-muted-foreground">
              Client ID: {client?.name ?? task?.client ?? '—'}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Phone className="w-4 h-4 text-[#3B82F6] shrink-0" />
            <span className="text-[13px] text-muted-foreground">
              {maskPhone(client?.mobile_number ?? client?.phone ?? '')}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Mail className="w-4 h-4 text-[#3B82F6] shrink-0" />
            <span className="text-[13px] text-muted-foreground">
              {maskEmail(client?.email_address ?? client?.email ?? '')}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] text-muted-foreground">
              {client?.address_line ?? client?.city
                ? `${client?.address_line ?? ''} ${client?.city ?? ''} Pincode - ${client?.pincode ?? ''}`
                : '—'}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge status={task?.status ?? 'On Going'} />
        </div>
        ...
        <div className="relative pl-6 mb-6">
          ...
          <div className="space-y-5">
            {steps.map((step, idx) => {
              const stepCompleted = (step.status ?? '').toLowerCase().includes('completed');
                  ...
                  <div className="flex-1 flex items-center justify-between">
                    <div>
                      <p className="text-[14px] font-bold text-green-600">
                        {step.step_label ?? `Step ${step.step_number ?? idx + 1}`}
                      </p>
                    ...
                    {isUpdateTarget ? (
                      <button
                        ...
                        className="flex items-center gap-1 bg-[#1A3C8E] text-white text-[13px] font-medium px-4 py-1.5 rounded-lg shrink-0"
                      >
        ...
        {isCompleted ? (
          <button className="w-full border border-[#3B82F6] text-[#3B82F6] rounded-xl h-12 text-[15px] font-medium">
            View Remark
          </button>
        ) : (
          <div className="space-y-3">
            <div className="flex gap-3">
              <button
                onClick={() => navigate(`/document-upload?task=${taskId}`)}
                className="flex-1 border border-[#3B82F6] text-[#3B82F6] rounded-xl h-12 text-[15px] font-medium flex items-center justify-center gap-1.5"
              >
                <Upload className="w-4 h-4" /> Upload
              </button>
              <button
                onClick={handleSendForApproval}
                disabled={!allStepsDone || sending}
                className="flex-1 bg-[#3B82F6] text-white rounded-xl h-12 text-[15px] font-medium flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
            ...
            <button
              onClick={handleSave}
              className="w-full bg-[#1A3C8E] text-white rounded-xl h-12 text-[15px] font-bold"
            >
              Save
            </button>
          </div>
