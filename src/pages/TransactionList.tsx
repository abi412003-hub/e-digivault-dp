...
import { ArrowLeft, MessageSquare, Bell, User, Plus, Wallet, ChevronDown, ChevronUp, Building2, AlertTriangle, Zap } from 'lucide-react';
...
function groupByDate(items: Transaction[]): [string, Transaction[]][] {
  ...
  items.forEach((item) => {
    const key = fmtDate(item.transaction_date) || 'Unknown Date';
            ...
            grouped.map(([date, items]) => (
              <div key={date}>
                ...
                <div className="space-y-2">
                  {items.map((tx) => {
                    const style = TYPE_STYLES[tx.transaction_type] ?? TYPE_STYLES.Received;
                    ...
                    return (
                      <div key={tx.name} className="bg-background border border-border rounded-xl shadow-sm overflow-hidden">
                        <button onClick={() => setExpandedId(expanded ? null : tx.name)}
